...
esp_ble_ota_host_init(void)
{
    ...
    /* Set the device name (ERR-006: was hardcoded "nimble-ble-ota"; now
     * app-configurable via Kconfig so the final name is written here, before
     * nimble_port_freertos_init() starts the host task whose sync_cb builds
     * the advertising payload from this name -- no race with app_main). */
    rc = ble_svc_gap_device_name_set(CONFIG_BLE_OTA_DEVICE_NAME);
    assert(rc == 0);
