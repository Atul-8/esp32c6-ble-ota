...
- TASK-005/PR-1 [完成] ota_sink 抽象 + BLE transport 迁移 + epoch 会话修复（REQ-004）：ota_core 新增 ota_sink.c/.h（会话状态机/epoch 单调分配/单写者先到先得/延迟 abort 并发模型/事件回调；选槽改 running partition 修 P1-5、size 校验收拢修 P1-4、begin 时机后移到 open 修 P1-7 开机全擦、idf_err 上抛修 P2-9）；main/ota_task.c/.h 删除，职责迁入 ble_transport.c/.h（ringbuf+泵任务+lazy-open+会话回调对接；notify_sem 非 static 保持 META-001 契约，初值 1 永不删除修 R8/P2-10）；vendor ble_ota 补丁 ~25 行（START/STOP/DISCONNECT 三会话边界各一行回调 + set_session_cb，组件保持会话无感知）；host P0-2 判定改广播空窗观测 + P2-14 Stop 3s 短超时；新增 ota_core/test/host_test_ota_sink.py 21 用例 21/21 PASS；tools/serial_read.py 串口捕获辅助 (涉及: firmware/components/ota_core/*, firmware/main/ble_transport.c/.h, firmware/main/main.c, firmware/main/CMakeLists.txt, firmware/components/ble_ota/include/ble_ota.h, firmware/components/ble_ota/src/nimble_ota.c, host/ble_ota_host.py, tools/serial_read.py)
- TASK-005/PR-1 [修复] ERR-013（真机回归 B 暴露三缺陷）：①armed 双写者——泵任务 STALE 路径清 armed 致重试 Start 的 lazy-open 永不触发（传输空转）；②ringbuf 残留 straggler 窗口（旧数据进新会话 offset 0）；③host"Stop ACK 正常返回=未重启"判据被真实收尾时序证伪（Stop 空口往返可抢在 esp_restart 前被 ACK）。修复：armed 所有权收敛到会话回调单写者、START 回调非阻塞排空 ringbuf、泵任务 100ms 超时路径收敛流死亡、open_blocked 数据源门控、host 判定改广播空窗观测（9s 窗口出现广播消失=重启）；提炼 META-010-CONCURRENCY (涉及: firmware/main/ble_transport.c, host/ble_ota_host.py, .ai/errors/raw/ERR-013.md)
- TASK-005/PR-1 [验证] 真机回归三连全绿：A 基础路径 161 sectors/13.6KB/s → finish ok → activated → 重启 PENDING_VERIFY→confirmed（槽位交替 ota_1→ota_0→ota_1 两轮）；B 断连重试（传输 ~30% 杀 host 进程）修复前=设备不重启+二次 open 缺失，修复后 epoch=1→disconnect→session abort by ble(20ms)→新 Start→epoch=2→161 sectors 全量→重启成功，两轮复验全绿；C 开机无全擦（启动日志无 esp_ota_begin/erase，adv started 457ms，对比迁移前开机 esp_ota_begin(OTA_SIZE_UNKNOWN) 全擦 1.75MB）；dry-run/scan 回归绿；构建零错误 bin=658,896B（+2KB）
...
- REQ-004 [决策] OTA 传输层抽象设计定稿：ota_sink（ota_core 新增，会话状态机/epoch 会话代数/单写者互斥/进度事件）+ 三 transport（ble 桥接不动 vendor 主干/wifi=esp_http_client 流式+Range 续传/usb=USJ 自定义帧协议）；硬件事实核实纠偏——C6 无 USB-OTG（TinyUSB 物理不可行）、USB 引脚=GPIO12/13（非 18/19，那是 C3）、IDF v6 有 esp_ota_resume 支持 WiFi 断点续传、esp_https_ota 无 write 注入口故否决；P0-1 修复归属 PR-1（epoch 机制=抽象组成部分）；ADR-004-1~5 留痕 (涉及: .ai/docs/design-ota-transport-abstraction.md)
- 20:10 [完成] PR-1 冒烟复核 PASS：21/21 单测复跑、回归 A 独立复跑 SUCCESS、回归 B（杀进程→abort→epoch 2）P0-1 实证、ota_task 删净、github 补推 3 提交
- 20:30 [修复] ERR-014 空窗判定器误报（1s 探测横跨 2s 重启全程）→ Stop 前基线 + 窗口尾部兜底；真机回归 OTA SUCCESS
